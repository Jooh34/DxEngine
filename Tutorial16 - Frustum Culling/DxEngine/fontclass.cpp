...
//Initialize will load the font dataand the font texture.
...
//Shutdown will release the font dataand the font texture.
...
//The LoadTexture function reads in the font.dds file into the texture shader resource.This will be the texture we take the characters fromand write them to their own square polygons for rendering.
...
void FontClass::BuildVertexArray(void* vertices, char* sentence, float drawX, float drawY)
{
	...
	index = 0;
	//The following loop will now build the vertexand index arrays.It takes each character from the sentenceand creates two triangles for it.It then maps the character from the font texture onto those two triangles using the m_Font array which has the TU texture coordinatesand pixel size.Once the polygon for that character has been created it then updates the X coordinate on the screen of where to draw the next character.
