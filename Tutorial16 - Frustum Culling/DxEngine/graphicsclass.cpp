...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), (char*) "../DxEngine/data/sphere.txt", (WCHAR*) L"../DxEngine/data/seafloor.dds");
	if (!result)
	{
...
bool GraphicsClass::Render(float rotation)
{
	...
	for (index = 0; index < modelCount; index++)
	{
		...
		radius = 1.0f;
		//Here is where we use the new FrustumClass object.We check if the sphere is viewable in the viewing frustum.If it can be seen we render it, if it cannot be seen we skip itand check the next one.This is where we will gain all the speed by using frustum culling.
