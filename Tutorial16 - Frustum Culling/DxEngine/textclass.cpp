	...
	XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char*)"../DxEngine/data/fontdata.txt", (WCHAR*)L"../DxEngine/data/font.dds");
	if (!result)
	{
...
bool TextClass::SetRenderCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	// Setup the cpu string.
	strcpy_s(cpuString, "Render Count:");
	strcat_s(cpuString, tempString);
...
//The InitializeSentence function creates a SentenceType with an empty vertex buffer which will be used to storeand render sentences.The maxLength input parameter determines how large the vertex buffer will be.All sentences have a vertexand index buffer associated with them which is initialized first in this function.
...
//UpdateSentence changes the contents of the vertex buffer for the input sentence.It uses the Mapand Unmap functions along with memcpy to update the contents of the vertex buffer.
...
//ReleaseSentence is used to release the sentence vertexand index buffer as well as the sentence itself.
