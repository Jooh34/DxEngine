...
bool MultiTextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"../DxEngine/multitextureVS.hlsl", (WCHAR*)L"../DxEngine/multitexturePS.hlsl");
	if (!result)
	{
		return false;
	}
...
//The InitializeShader function loads the vertexand pixel shader as well as setting up the layout, matrix buffer, and sample state.
...
//SetShaderParameters sets the matricesand texture array in the shader before rendering.
